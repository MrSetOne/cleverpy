...
const Post = ({post, i}:Props) => {
  ...
  return (
    <motion.article className='post' variants={variants} initial='initial' animate={tools?'open':'initial'} whileHover={{scale:1.03}}>
      <header className="post__header">
        ...
        <div className="post__btns">
          {
            author&&
            <button className="post__btns--edit" onClick={()=>setTools(tools!=='edit'&&'edit')}>
              <FontAwesomeIcon icon={faPenToSquare} />
            </button>
          }
          {
            logged&&
              <button className="post__btns--delete" onClick={()=>setTools(tools!=='delete'&&'delete')}>
